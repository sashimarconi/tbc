...
async function resolvePublicOwnerContext(req, slug, options = {}) {
  const { activeOnlyBase = true, strictDomainOwner = false } = options;
  ...
  let baseProduct = await resolveBaseBySlug(normalizedSlug, domainOwnerUserId, activeOnlyBase);

  if (!baseProduct && domainOwnerUserId && !strictDomainOwner) {
    // Fallback for shared/platform domains accidentally present in custom_domains:
    // if slug exists globally, prefer not to hard-fail by owner pinning.
    baseProduct = await resolveBaseBySlug(normalizedSlug, null, activeOnlyBase);
  }
