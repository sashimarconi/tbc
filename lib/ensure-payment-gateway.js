...
async function ensurePaymentGatewayTable() {
  ...
  // Ensure column exists for upgrades where the table was created before product_hash was added
  await query("alter table user_payment_gateways add column if not exists product_hash text");
  ...
  await query(`
    create table if not exists user_payment_gateway_settings (
      owner_user_id uuid primary key references users(id) on delete cascade,
      selected_provider text not null default 'sealpay',
      updated_at timestamptz not null default now()
    )
  `);
  await query(
    "create unique index if not exists user_payment_gateway_settings_owner_uidx on user_payment_gateway_settings (owner_user_id)"
  );
