...
function respondDegraded(res, payload = {}) {
  res.status(200).json({
    ok: false,
    degraded: true,
    ...payload,
...
module.exports = async (req, res) => {
  ...
  try {
    ownerContext = await resolvePublicOwnerContext(req, slug, { activeOnlyBase: true });
  } catch (error) {
    console.error("[public/cart] failed to resolve owner context", {
      slug,
      cartKey,
      error: error?.message,
    });
    respondDegraded(res, { cart_id: cartKey, slug, stage: ensureStage(body.stage) });
    return;
  }
  ...
  try {
    ...
    respondDegraded(res, { cart_id: cartKey, slug, stage, status });
  }
};
